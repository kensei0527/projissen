...
abstract class MovingObject { // 抽象クラス
  ...
  MovingObject (int width, int height) {
    ...
    y  = (int) (Math.random()*height/2); // 画面の内部でランダム
    dx = rand.nextInt(10) - 5;   // -3から+3の範囲でランダム
    /*dy = (int) (Math.random()*2);    // -3から+3の範囲でランダム
    while(dy == 0){
  ...
  //abstract void bullet(int apWidth, int apHeight);
